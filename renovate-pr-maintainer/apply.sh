...
if [ -n "${GITHUB_STEP_SUMMARY:-}" ]; then
...

  {
    ...
    printf '\n**Columns**\n\n- **state** — GitHub `mergeable_state`, plus synthetic states this action adds (`rebase-labeled`, `queued`); the ones worth context: `blocked` (a required gate unmet — a failing/pending required check *or* a missing required review), `unstable` (only a *non-required* check is failing/pending, so GitHub still allows merge), `behind` (head behind base), `dirty` (merge conflict), `queued` (in the GitHub merge queue, being merged — GitHub owns it).\n- **blockers** — why the PR is not merge-ready now: failing/pending required check, awaiting required review, merge conflict, behind base, in merge queue; `—` = nothing blocking.\n- **automerge** — `yes` when the PR is set to auto-merge once green (drives `require-up-to-date-strategy`).\n- **behind_by** — commits the head is behind base; `—` when not measured (indeterminate `unknown` or `queued` state).\n- **head age (h)** — hours since the head was pushed: while behind base it is the staleness clock a rebase resets; for `dirty`/`rebase-labeled` PRs it is the stuck clock (how long the PR has sat conflicted or awaiting Renovate); `—` when not measured.\n- **action** — what the maintainer did, outcome in parens: `applied`, `dry-run`, `failed`, or `deferred` (past the `batch-size`=%s cap, retried next run); `none`/`skip`/`pending` change nothing.\n\n**How it works** — **rebases** stale behind PRs (≥ %s commits behind or ≥ %sh old) and **re-runs** failing required checks (`rerun-budget`=%s). Full [decision model](%s).\n' \
